...
const from = (_i: number) => ({ y: 0, rot: 0, scale: 1.5, x: 300 })
...
const Spring = () => {
  ...
  const bind = useDrag(({ args: [index], down, movement: [mx], direction: [xDir], velocity }) => {
    ...
    api.start(i => {
      ...
      const x = isGone ? 350 * dir : down ? mx : 0 // When a card is gone it flys out left or right, otherwise goes back to zero
